    ...
    if submitted:
        if not selected_dishes:
            ...
            if os.path.exists(pdf_path):
                os.remove(pdf_path)import streamlit as st
import pandas as pd
import os
import smtplib
from email.mime.multipart import MIMEMultipart
...
def generate_pdf(shopping_df, name, firstname, address=None, num_guests=4, selected_dishes=None):
    ...
    for i, (plat, group) in enumerate(shopping_df.groupby("Plat", sort=False)):
        ...
        # Diviser en 2 colonnes si plus de 4 ingrédients
        if len(ing_data) > 4:
            mid = (len(ing_data) + 1) // 2
            left_col = ing_data[:mid]
            right_col = ing_data[mid:]
            # Rembourrer la colonne droite si nécessaire
            while len(right_col) < len(left_col):
                right_col.append([Paragraph("", style_ing_cell), Paragraph("", style_qty_cell)])

            two_col_data = []
            for l, r in zip(left_col, right_col):
                two_col_data.append(l + [Paragraph("│", ParagraphStyle(
                    'sep', parent=styles['Normal'], fontSize=10,
                    textColor=OR_CLAIR, alignment=TA_CENTER
                ))] + r)

            ing_table = Table(two_col_data, colWidths=[6.5*cm, 2.5*cm, 0.5*cm, 6.5*cm, 1*cm])
        else:
            ing_table = Table(ing_data, colWidths=[12*cm, 5*cm])
    ...
    elements.append(Paragraph(
        "La Valise aux Épices — Cuisine faite maison, livrée avec amour 🥘",
        footer_style
    ))
